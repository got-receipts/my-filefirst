...
def render_nav(user, cart_count=0):
    ...
    if user:
        ...
        if user["role"] == "client":
            links.append(f'<a href="/cart">Bag ({cart_count})</a>')
        if user["role"] == "admin":
...
def render_cart_page(connection, user, message=None, level="info"):
    ...
    <section class="cart-layout">
      <section class="panel">
        <div class="panel-head">
          <h2>Your Bag</h2>
          <a class="button ghost" href="/">Back to Menu</a>
        </div>
...
def handle_add_to_cart(environ, start_response, connection, user):
    ...
    product = connection.execute("SELECT * FROM products WHERE id = ?", (product_id,)).fetchone()
    if not product:
        return redirect(start_response, "/?message=Menu item not found")
    if quantity < 1 or quantity > product["stock"]:
        return redirect(start_response, "/?message=Please choose a valid quantity")
    ...
    connection.commit()
    return redirect(start_response, "/cart?message=Added to bag")
...
def handle_remove_from_cart(environ, start_response, connection, user):
    ...
    if gate:
        return gate
    product_id = int(read_post_data(environ).get("product_id", "0"))
    connection.execute("DELETE FROM cart_items WHERE user_id = ? AND product_id = ?", (user["id"], product_id))
    connection.commit()
    return redirect(start_response, "/cart?message=Item removed")
...
def handle_cart_checkout(environ, start_response, connection, user):
    ...
    items = cart_items_for_user(connection, user["id"])
    if not items:
        return redirect(start_response, "/cart?message=Your bag is empty")
    fulfillment_type = data.get("fulfillment_type", "DELIVERY")
    shipping_address = data.get("shipping_address", "").strip()
    if fulfillment_type == "DELIVERY" and not shipping_address:
        return redirect(start_response, "/cart?message=Delivery address is required")
    if fulfillment_type == "PICKUP" and not shipping_address:
        shipping_address = "In-store pickup"
    try:
        create_ticket(
        ...
        return redirect(start_response, f"/cart?message={str(exc)}")
...
def application(environ, start_response):
    ...
    with db_connection() as connection:
        ...
        message = query_params(environ).get("message")
        ...
        if path == "/" and method == "GET":
            return text_response(start_response, render_store_page(connection, user=user, message=message))
        if path == "/login":
